...
SELECT
    DISTINCT  TOP (5) @@SERVERNAME AS [Server Name], 
...
WHERE 
    b.blocking_session_id <> 0 and b.blocking_session_id <>b.session_id 
	 AND (b.wait_duration_ms / 1000) / 60 >= 1 --changed here --add the condition
	order by  (b.wait_duration_ms / 1000)/ 60 desc;
...
GROUP BY 
    ...
    t.login_name
	HAVING 
    DATEDIFF(SECOND, MAX(ses.last_request_start_time), GETDATE()) >= 60
...
SET @subject = N'Invoicemart ' + @ServerName + ' - Blocking Queries -> Open'; --Change 1
...
(SELECT TOP (5)
    ...
    WHERE DATEDIFF(SECOND, b.logdate, GETDATE()) < 60 and b.[WaitingInMinutes]>=1
...
SELECT @Summary = (
    SELECT 
     ...
     FROM dbadb..HeadBlockingInfo s
        WHERE DATEDIFF(SECOND, logdate, GETDATE()) < 60   -- logged within this job run
    FOR XML PATH(''), TYPE).value('.', 'NVARCHAR(MAX)');
	
	--comment out the old condition
    ...
    --HAVING DATEDIFF(SECOND, MAX(ses.last_request_start_time), GETDATE()) >= 60
   -- FOR XML PATH(''), TYPE).value('.', 'NVARCHAR(MAX)');
...
'<p>Best Regards,</p>' +
'<p>MSSQL DBA<br>Rapyder</p>' +  -- change 2
'</body>' +
'</html>';
...
@profile_name = 'RDSMail',     --Change 3 DBmail Profiler
@body = @HTML,
@body_format ='html',
@recipients = 'atreds.dbalert@invoicemart.com;tpat00223@invoicemart.com;sujeet1.pawar@invoicemart.com;pankaj.bhalekar@invoicemart.com;durgesh.chaubey@invoicemart.com;vidyashree.samani@invoicemart.com',  -- change 4 Our mail or client mail
...
(SELECT TOP(5)
       ...
       '<td>' +CAST( b.WaitType AS NVARCHAR) + '</td>' +
        ...
        '</tr>'
    FROM dbadb..BlockedQueriesInfo b
    WHERE DATEDIFF(HOUR, b.logdate, GETDATE()) <= 1 and b.[WaitingInMinutes]>=1 and is_closed=0
...
--Select @HTML_Closed;
 SET @subject = N'Invoicemart ' + @ServerName + ' - Blocking Queries -> Closed'; --Change 1
...
@body = @HTML_Closed,
@body_format = 'html',
@recipients = 'atreds.dbalert@invoicemart.com;tpat00223@invoicemart.com;sujeet1.pawar@invoicemart.com;pankaj.bhalekar@invoicemart.com;durgesh.chaubey@invoicemart.com;vidyashree.samani@invoicemart.com',
@copy_recipients ='mssqlsupport@rapyder.com', -- change 4 Client Mail or Our alert mail
 @subject =@subject
END
